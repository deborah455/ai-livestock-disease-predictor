...
# - Model: loads results/vision/best.pth by default (or TorchScript if asked)
# - Output: prints decisions, saves annotated images + predictions.csv

# Quiet optional CPU backends & warning spam (safe; does not change accuracy)
import os
...
def prob_unhealthy(model, img_tensor: torch.Tensor) -> float:
    with torch.no_grad():
        ...
        return float(probs[1])      # class 1 = UNHEALTHY
...
def annotate(image_path: Path, out_path: Path, label: str, p: float, thresh: float):
    ...
    thickness = max(4, min(W, H)//200)
    for t in range(thickness):
        draw.rectangle([t, t, W-1-t, H-1-t], outline=color)
    bar_h = max(24, H//18)
    draw.rectangle([0, 0, W, bar_h], fill=color)
    text = f"{label}  p_unhealthy={p:.3f}  (τ={thresh:.2f})"
    try:
        font = ImageFont.load_default()
    except Exception:
        font = None
    draw.text((8, max(2, bar_h//8)), text, fill=(255, 255, 255), font=font)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    im.save(out_path)

def smooth_majority(decisions: List[int], k: int, m: int) -> List[int]:
    if k <= 1: return decisions
    from collections import deque
    dq = deque(maxlen=k)
    out = []
    for d in decisions:
        dq.append(d)
        out.append(1 if sum(dq) >= m else 0)
    return out
...
def main():
    ...
    ap.add_argument("--scripted", default="results/vision/model_scripted.pt", help="Path to TorchScript .pt")
    ap.add_argument("--use_scripted", action="store_true", help="Use TorchScript instead of checkpoint")
    ap.add_argument("--out", default="results/vision_infer", help="Output folder (annotated images + CSV)")
    ap.add_argument("--threshold", type=float, default=0.50, help="Decision threshold τ on p_unhealthy")
    ap.add_argument("--vote_k", type=int, default=1, help="Temporal smoothing window K (>=1)")
    ap.add_argument("--vote_m", type=int, default=1, help="Votes needed M within window (<=K)")
    args = ap.parse_args()
    ...
    for i, p in enumerate(files, 1):
        ...
        pred = int(punh >= args.threshold)  # 1=UNHEALTHY
        raw_decisions.append(pred)
        ...
        rows.append({
            "index": i,
            "path": str(p),
            "p_unhealthy": round(punh, 6),
            "prediction_raw": label
        })
        print(f"[{i:04d}] {p.name}  p_unhealthy={punh:.3f}  → {label}")

    # Optional temporal smoothing (for frame sequences)
    if args.vote_k > 1:
        m = min(args.vote_m, args.vote_k)
        sm = smooth_majority(raw_decisions, k=args.vote_k, m=m)
        for r, s in zip(rows, sm):
            r["prediction_smoothed"] = "UNHEALTHY" if s == 1 else "HEALTHY"
        print(f"\n[i] Applied smoothing: K={args.vote_k}, M={m}")

    # Save CSV
    outdir = Path(args.out); outdir.mkdir(parents=True, exist_ok=True)
    csv_path = outdir / "predictions.csv"
    pd.DataFrame(rows).to_csv(csv_path, index=False)
    unhealthy_cnt = sum(1 for r in rows if r["prediction_raw"] == "UNHEALTHY")
